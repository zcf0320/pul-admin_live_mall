...
const OperateModel: FC<IProps> = (props: IProps) => {
  ...
  const columns: any[] = [
    ...
    },
    {
      ...
      render: (_: any, record: TableListItem) => {
        return (
          // 系统默认账号和自己不可以选择
          !record?.isDefault && record?.id !== parentId ? (
            <Button
              type="link"
              onClick={() => {
                setSelectObj({
  ...
  const setAccountInfo = async () => {
    ...
    const params: any = {
      [texts?.currentIdName]: id ? [id] : selectRowKeys,
      [keyName]: otherId,
    };
    if (!setMethod) return;
    try {
  ...
  }, [requestParams, isShowSelect, showOperate]);

  useEffect(() => {
    formRef.current?.setFieldValue(texts?.optionName, isDefaultTeam ? '2' : '1');
    setIsShowSelect(isDefaultTeam);
  }, [showOperate]);
      ...
      <div className={styles.remarkModel}>
        ...
        >
          <div className={styles.remarkModelInfo}>
            {/* 团长信息：{name ?? '-'}（ID:{id ?? '-'}） */}
            已选:
            <span style={{ color: '#2e74ff', margin: '0 10px' }}>{selectRowKeys.length}</span>人
          </div>
        </Popover>
